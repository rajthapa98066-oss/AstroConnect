...
<section class="mx-auto max-w-7xl px-4 pb-16 pt-14 sm:px-6 lg:px-8 lg:pb-24 lg:pt-20">
    <div class="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
            <p class="text-sm uppercase tracking-[0.35em] text-amber-200/70">Astrologer Studio</p>
            <h1 class="mt-4 text-5xl text-white sm:text-6xl [font-family:'Cormorant_Garamond',serif]">Refine your public profile and attract your ideal clients.</h1>
            <p class="mt-6 max-w-2xl text-lg leading-8 text-slate-300">Keep your specialization, availability, and consultation details fresh so seekers can quickly understand your style and offerings.</p>
        </div>
        ...
        <div class="rounded-[2rem] border border-white/10 bg-white/5 p-8 backdrop-blur">
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="rounded-2xl border border-white/10 bg-slate-950/70 p-5">
                    ...
                    <p class="mt-2 text-2xl text-amber-200 [font-family:'Cormorant_Garamond',serif]">{{ ucfirst($astrologer->availability_status) }}</p>
                </div>
                <div class="rounded-2xl border border-white/10 bg-slate-950/70 p-5">
...
<section class="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
    <div class="rounded-[2rem] border border-white/10 bg-gradient-to-b from-slate-900 to-slate-950 p-7 shadow-xl shadow-slate-950/30 sm:p-8">
        ...
        <form method="POST" action="{{ route('astrologer.profile.update') }}" enctype="multipart/form-data" class="grid gap-6 lg:grid-cols-2">
            @csrf
            @method('PATCH')
            ...
            <div class="lg:col-span-2">
                ...
                @if ($astrologer->profile_photo)
                    <p class="mt-2 text-sm text-slate-300">Current photo is available in your public listing.</p>
                @endif
            </div>
