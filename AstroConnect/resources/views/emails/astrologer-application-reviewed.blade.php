...
<head>
    ...
    <title>Astrologer Application Review</title>
    <style>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="container">
                    ...
                    <tr>
                        <td class="content">
                            ...
                            @if(($status ?? '') === 'approved')
                                <h2>Congratulations, your astrologer profile is approved.</h2>
                                <p>You can now access astrologer tools and manage your sessions.</p>
                            @else
                                <h2>Your astrologer application was reviewed.</h2>
                                <p>Your current application status has been updated by the admin team.</p>
                            @endif
                            <div class="box">
                                <p><strong style="color:#fff7ed;">Current status:</strong> {{ $status ?? 'Pending' }}</p>
                                <p><strong style="color:#fff7ed;">Specialization:</strong> {{ $specialization ?? 'N/A' }}</p>
                            </div>
                            <div class="cta">
                                <a href="{{ $actionUrl }}" class="btn">{{ $actionText ?? 'View Application' }}</a>
                            </div>
                            <p style="margin-top:18px;color:#94a3b8;">{{ $outro ?? 'You can update your profile and submit again if needed.' }}</p>
                        </td>
                    </tr>
                    <tr>
                        <td class="footer">
