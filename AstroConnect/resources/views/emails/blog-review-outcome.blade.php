...
<head>
    ...
    <title>Blog Review Outcome</title>
    <style>
        ...
        <tr>
            <td align="center">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="container">
                    ...
                    <tr>
                        <td class="content">
                            <p class="eyebrow">Blog outcome</p>
                            @if(($status ?? '') === 'approved')
                                <h2>Your blog was approved.</h2>
                                <p>Your content has passed moderation and can now be managed in your blog panel.</p>
                            @else
                                <h2>Your blog was not approved.</h2>
                                <p>Your submitted blog has been reviewed and currently remains unpublished.</p>
                            @endif
                            <div class="box">
                                <p><strong style="color:#fff7ed;">Title:</strong> {{ $title ?? 'N/A' }}</p>
                                <p><strong style="color:#fff7ed;">Status:</strong> {{ $status ?? 'Pending' }}</p>
                            </div>
                            <div class="cta">
                                <a href="{{ $actionUrl }}" class="btn">{{ $actionText ?? 'Open My Blogs' }}</a>
                            </div>
                            <p style="margin-top:18px;color:#94a3b8;">{{ $outro ?? 'Please revise and resubmit your blog when ready.' }}</p>
                        </td>
                    </tr>
                    <tr>
                        <td class="footer">
